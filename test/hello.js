...
describe("hello returnValue true", function() {
  const mod = { };
  const context = {
    query: "?locales[]=en&locales[]=en-US&locales[]=fr&returnFunction=true",
    resourcePath: path.resolve(__dirname, "nls", "hello_nls.json"),
    addDependency: function () { },
  };
  const source = JSON.stringify(helloNLS);
  ...
  it("should return default", function() {
    def.call({ }, mod);
    assert.equal(JSON.stringify(mod.exports("pt")), JSON.stringify(allHelloNLS.root));
  });

  it("should return english", function() {
    def.call({ }, mod);
    assert.equal(JSON.stringify(mod.exports("en")), JSON.stringify(allHelloNLS.en));
  });

  it("should still return english", function() {
    def.call({ }, mod);
    assert.equal(JSON.stringify(mod.exports("en-IN")), JSON.stringify(allHelloNLS.en));
  });

  it("should return american english", function() {
    def.call({ }, mod);
    assert.equal(JSON.stringify(mod.exports("en-US")), JSON.stringify(allHelloNLS["en-US"]));
  });

  it("should return french", function() {
    def.call({ }, mod);
    assert.equal(JSON.stringify(mod.exports("fr")), JSON.stringify(allHelloNLS.fr));
  });
});
